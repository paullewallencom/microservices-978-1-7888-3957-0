...
from lambda_dynamo_xray import lambda_return_dynamo_records as lambda_query_dynamo
...
with open('../sample_data/request-api-gateway-valid-no-date.json', 'r') as sample_file:
    event = json.loads(sample_file.read())
print(sample_file.name.split('/')[-1])
print("lambda_query_dynamo\nUsing data: " + str(event))
response = lambda_query_dynamo.lambda_handler(event, None)
...
with open('../sample_data/request-api-gateway-error.json', 'r') as sample_file:
    event = json.loads(sample_file.read())
print(sample_file.name.split('/')[-1])
print("lambda_query_dynamo\nUsing data: " + str(event))
response = lambda_query_dynamo.lambda_handler(event, None)
